...
MessageType NetworkMessage::typeFromString(const std::string& name)
{
    if (name == "GAME_STATE")
...
void NetworkMessage::setPayload(const std::string& payload)
{
    m_payload = payload;
}

void NetworkMessage::setPayload(std::string&& payload)
{
...
std::optional<NetworkMessage> NetworkMessage::deserialize(const std::string& data)
{
    ...
    std::string type = data.substr(0, separator);
    std::string payload = data.substr(separator + 1);

    MessageType messageType = typeFromString(type);
...
{
    if (m_type == MessageType::UNKNOWN)
    {
        return false;
    }

    if (m_type == MessageType::DISCONNECT || m_type == MessageType::HEARTBEAT)
    {
        return true;
    }
...
{
    return m_type == MessageType::COMMAND;
}

bool NetworkMessage::isCommandResult() const
{
    return m_type == MessageType::COMMAND_RESULT;
}

bool NetworkMessage::isGameState() const
{
    return m_type == MessageType::GAME_STATE;
}

bool NetworkMessage::isError() const
{
    return m_type == MessageType::SYSTEM_ERROR;
}

bool NetworkMessage::isDisconnect() const
{
    return m_type == MessageType::DISCONNECT;
}

bool NetworkMessage::isHeartbeat() const
{
    return m_type == MessageType::HEARTBEAT;
}