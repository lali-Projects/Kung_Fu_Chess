...
bool SQLiteDatabase::open()
{
    ...
    int result = sqlite3_open(m_filename.c_str(), &m_database);
    ...
    if (result != SQLITE_OK)
    {
        ...
        {
            m_lastError = "Failed opening database";
        }
...
bool SQLiteDatabase::execute(const std::string& sql, const std::vector<std::string>& params)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    ...
    int prepareResult = sqlite3_prepare_v2(m_database, sql.c_str(), -1, &statement, nullptr);
    ...
    if (!bindParameters(statement, params))
    {
...
std::optional<DatabaseRow> SQLiteDatabase::queryOne(
