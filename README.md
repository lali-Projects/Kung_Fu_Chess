...
* Authentication, login, logout, room join, and leave handling
* Server-side player ownership validation
* Server-side spectator authorization

Main Components: `Server`, `WebSocketServer`, `ConnectionManager`, `ClientConnection`, `CommandHandler`, `RoomManager`, `GameSession`, `PlayerLifecycleService`, `AuthoritativeGameLoop`
...
A room supports exactly one WHITE player, one BLACK player, and any number of authenticated observers.

Observers receive the same live authoritative game-state updates as active players, but cannot select pieces, move pieces, or modify the game state.

If an active player disconnects, the remaining observers stay connected as OBSERVER. A newly joining client can claim the vacant active side.
